...
sealed interface AppStatusBarUiState {
    data object Idle : AppStatusBarUiState
    data object Loading : AppStatusBarUiState
    data class Success(val message: String) : AppStatusBarUiState
    data class Error(val error: DataError) : AppStatusBarUiState
}

sealed interface StoryListUiState {
    data object Idle : StoryListUiState
    data object Loading : StoryListUiState
    data class Success(val stories: List<StoryBaseInfo>) : StoryListUiState
    data class Error(val error: DataError) : StoryListUiState
}
...
class StoryListViewModel(
    ...
    private val _uiState = MutableStateFlow<StoryListUiState>(StoryListUiState.Idle)
                    ...
                    is StoryListUiState.Success -> {
                        when (adsState) {
                            is NativeAdManager.AdsState.Loaded -> {
                                ...
                                updateMixedList(storiesState.stories, adsState.ads)
                            }
                            is NativeAdManager.AdsState.Empty -> {
                                Log.d(">>> Story List VM <<<", "No ads available yet")
                                updateMixedList(storiesState.stories, emptyList())
                                loadAds()
                            }
                            else -> {
                                updateMixedList(storiesState.stories, emptyList())
    ...
    override fun onCleared() {
        super.onCleared()
//        currentJob?.cancel() // vmScope отменяет корутины
